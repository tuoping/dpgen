...
def inter_mace(param):
    ...
    ret = "pair_style mace no_domain_decomposition\n"
    model_list = ""
    for ii in models:
        model_list += ii + " "
    type_str = " ".join(list(param["param_type"].keys()))
    ret += f"pair_coeff * * {model_list}  {type_str}\n"
    return ret
...
def make_lammps_eval(conf, type_map, interaction, param):
    ...
    if "map_yes" in param and "map_yes" in param:
