...
def run_md_model_devi(iter_index, jdata, mdata):
    ...
    print(">>> md_resources")
    print(mdata["model_devi_resources"]["custom_flags"])
    submission = make_submission(
...
def _read_model_devi_file(
    ...
    try:
        ...
        raise Exception("Failed read of model_devi.out in", task_path)
...
def _select_by_model_devi_standard(
    ...
    for tt in modd_system_task:
        with warnings.catch_warnings():
            ...
            if not os.path.exists( os.path.join(tt, "model_devi.out")):
                dlog.info(f"WARNINING:: {tt} didn't run")
                continue
                ...
                if cluster_cutoff is None:
                    ...
                    if (
                        ...
                        if detailed_report_make_fp:
                            fp_rest_failed.append([tt, cc])
                        counter["failed"] += 1
                    elif all_conf[ii][1] < v_trust_lo or all_conf[ii][4] < f_trust_lo:
                        if detailed_report_make_fp:
    ...
    for ss in system_index:
        ...
        if model_devi_engine in ("lammps", "gromacs", "calypso"):
            ...
            if not model_devi_adapt_trust_lo:
                ...
                # print(fp_rest_accurate)
                # print(fp_rest_failed)
                # print(fp_candidate)
                # print(counter)
                # raise RuntimeError
            else:
    ...
    for ss in system_index:
        ...
        for oo in sys_output:
            try:
                ...
                print("WARNING:: Wrong output from", f"task.{ss}.*/output")
                continue
            if sum(_sys['atom_numbs']) == 0:
                continue
            assert 'virials' in _sys.data.keys()
            if np.any(np.abs(_sys['forces'])) > 10:
                continue
            all_sys.append(_sys)
            icount += 1
